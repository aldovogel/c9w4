...
shinyServer(function(input, output) {
    ...
    output$plot <- renderPlotly({
      ...
      #rn <- which(csv_bitstamp$Date ==input$date)
      #print(rn)
      ...
      #print(paste(dateMaxBtcClose, " highest"))
      #input$date
      ...
      #output$priceonday <- renderText({paste("On ", buydate, " the price of 1 BTC equalled $", print(btcPriceOnSelDay))})
      #output$profit <- renderText({paste("Buying in on ", buydate, " and spending $", input$buyamountusd, "until ", dateMaxBtcClose, "and then selling you would have made a profit of $", profit)})
      ...
      p <- plot_ly(data = csv_bitstamp, x = csv_bitstamp$Date[dayrange], y = csv_bitstamp$Close[dayrange], type = "scatter", mode = "lines", showlegend = TRUE, name = "BTC Closing Price") %>%
          ...
          #toggle for showing volume in plot
          #if(input$showvolume){
